...
from prox import inplace_prox, inplace_mask
import numpy as np
...
class Fssc(nn.Module):
    def __init__(self, input_dim, z_dim, encodeLayer=[], decodeLayer=[], 
            activation="relu", sigma=1., alpha=1., gamma=1., device="cuda", likelihood_type='zinb'):
        super(Fssc, self).__init__()
        self.z_dim = z_dim
        ...
        self.n_remove_pre = 0
        self.pre_w0_mask = torch.ones(input_dim, dtype=torch.bool).to(self.device)
        self.to(device)
        
    ########## prox ##########
    def input_mask(self):
        with torch.no_grad():
            return torch.norm(self.encoder[0].weight.data, p=2, dim=0) != 0

    def selected_count(self):
        return self.input_mask().sum().item()
    
 
    def prox(self, *, alpha, lambda1):
        with torch.no_grad():
            idx_remove = inplace_prox(
                beta=self.encoder[0],
                alpha_=alpha, lambda1_ = lambda1)
        return idx_remove   
    
    def w_mask(self, ):
        with torch.no_grad():
            inplace_mask(
                beta=self.encoder[0], mask = self.pre_w0_mask)     
    #########################
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (x_batch, x_raw_batch, sf_batch) in enumerate(dataloader):
                ...
                if self.likelihood_type == "nb":
                    loss = self.nb_loss(x=x_raw_tensor, mean=mean_tensor, disp=disp_tensor, scale_factor=sf_tensor)
...
#            acc = np.round(cluster_acc(y, y_pred), 5)
        ...
        for epoch in range(num_epochs):
            if epoch%update_interval == 0:
                ...
                if y is not None:
                    final_acc = acc = np.round(cluster_acc(y, y_pred), 5)
                    final_nmi = nmi = np.round(metrics.normalized_mutual_info_score(y, y_pred), 5)
                    final_ari = ari = np.round(metrics.adjusted_rand_score(y, y_pred), 5)
            ...
            for batch_idx in range(num_batch):
                ...
                if self.likelihood_type == "nb":
                    recon_loss = self.nb_loss(x=rawinputs, mean=meanbatch, disp=dispbatch, scale_factor=sfinputs)
                elif self.likelihood_type == "zinb":
                    recon_loss = self.zinb_loss(x=rawinputs, mean=meanbatch, disp=dispbatch, pi=pibatch, scale_factor=sfinputs)

                #w0_mask = self.input_mask().detach()
                #recon_loss = torch.mean(recon_loss[:, w0_mask])
                ...
                idx_remove = self.prox(alpha =  optimizer.param_groups[0]["lr"], 
                          lambda1 = self.lambda1)
                cluster_loss_val += cluster_loss.item() * len(inputs)
                recon_loss_val += recon_loss.item() * len(inputs)
                train_loss += loss.item() * len(inputs)
                
                if(idx_remove.shape[0] > self.n_remove_pre):
                    if(self.n_remove_pre == 0):
                        self.gene_removed = idx_remove.copy()
                    else:
                        assert(set(self.gene_removed) == set(idx_remove[:self.n_remove_pre]) )
                        self.gene_removed = np.concatenate([self.gene_removed, idx_remove[self.n_remove_pre:]])
                    self.n_remove_pre = idx_remove.shape[0]
                

            #print("Epoch %3d: Total: %.8f Clustering Loss: %.8f ZINB Loss: %.8f" % (
            #    epoch + 1, train_loss / num, cluster_loss_val / num, recon_loss_val / num))

        return y_pred, final_acc, final_nmi, final_ari